...
var init =   async function() { 
  const  visorSize  = localStorage.getItem("visorSize"),
       ...
       await  ajax({
                     ...
                     console.log(" Post Pagination:",posts);
...
const Pagination = {
   ...
   showMenu: function() {        
        ...
        if (Pagination.totalPage < Pagination.step * 2 + 6) {
            ...
            Pagination.add(Pagination.activePage - Pagination.step, (parseInt(Pagination.activePage) + parseInt(Pagination.step) + 1));
    ...
    inicio: function(){
        Pagination.activePage = 1;
         lS.setItem("oldActivePage", lS.getItem("activePage"));
         lS.setItem("activePage", Pagination.activePage);
        Pagination.showMenu();
    },
    prev: function() {    
      $("botonPrev").dataset.valor = Pagination.activePage;
    ...
    next: function() {
       $("botonNext").dataset.valor = Pagination.activePage;
        // alert("FooterPageButton(activePage):" +Pagination.activePage) ;
        Pagination.activePage++;       
        if (Pagination.activePage > Pagination.totalPage) {
             Pagination.activePage = Pagination.totalPage;
        }
