...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 28-03-2021 a las 11:23:14
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CartClient` (IN `idIn` INT)  READS SQL DATA
SELECT * FROM shoppingcart WHERE `idclient` = idIn$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `CartClientProduct` (IN `idClientIn` INT, IN `idProductIn` INT)  NO SQL
SELECT * FROM `shoppingcart` WHERE `idclient`= idClientIn AND `idproduct`= idProductIn$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `CartClientProductAmount` (IN `idClientIn` INT, IN `idProductIn` INT)  NO SQL
SELECT units * 2 FROM `shoppingcart` WHERE `idclient`= idClientIn AND `idproduct`= idProductIn$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `CartList` ()  READS SQL DATA
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ManageThePurchase` (IN `myIdClient` INT)  NO SQL
BEGIN
DECLARE message varchar(500) DEFAULT "";
CALL mobile_store_2021.ManageThePurchase(myIdClient,message);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `shopping_cart_view`  AS  select `s`.`idClient` AS `idclient`,`s`.`idProduct` AS `idproduct`,`m`.`modelName` AS `model`,`s`.`units` AS `units`,`p`.`salePrice` AS `price`,`m`.`imageModel` AS `photo` from ((`mobile_store_2021`.`shopping_cart` `s` join `mobile_store_2021`.`model` `m`) join `mobile_store_2021`.`shop_product` `p`) where `s`.`idProduct` = `p`.`idProduct` and `p`.`referencia_sku` = `m`.`referencia_sku` ;
